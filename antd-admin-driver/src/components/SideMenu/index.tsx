    ...
    menuList.forEach((itm, ind) => {
      if (itm.menuType === 1 && itm.menuState === 1) {
        if (itm.buttons?.length) {
          ...
          return treeList.push(getItem(itm.menuName, itm.path || ind, createIcon(itm.icon)))
        }
        treeList.push(getItem(itm.menuName, itm.path || ind, createIcon(itm.icon), getTreeMenu(itm.children || [])))
      }
    })
    return treeList
  }
